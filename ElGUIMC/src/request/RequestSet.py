...
class RequestSet(metaclass=Singleton):
    ...
    def find_docs_by_all_fields(self, finish_time, status, student_id, quantity=None, purpose=None, creation_time=None):
    ...
    def find_tsrs_by_all_fields(self, finish_time, status, student_id, problem=None, tsr_id=None, creation_time=None):
        return next((obj for obj in self.requests if obj.finish_time == finish_time and
                     obj.status == status and
                     int(obj.student_id) == int(student_id) and
