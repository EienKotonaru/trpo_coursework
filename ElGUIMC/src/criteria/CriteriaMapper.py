...
class CriteriaMapper(metaclass=Singleton):
    ...
    def insert(self, criteria_obj):
        ...
        cursor.execute("INSERT INTO Criterias (name, measure, group_id) VALUES (%s, %s, %s) RETURNING id;", (criteria_obj.name, criteria_obj.measure, criteria_obj.group_id))
        self.db.commit()
        criteria_obj.set_id(cursor.fetchone()[0])
        cursor.close()

    def update(self, criteria_obj):
        cursor = self.db.cursor()
        cursor.execute("UPDATE Criterias SET measure=%s WHERE id=%s;", (criteria_obj.measure, criteria_obj.group_id))
        self.db.commit()
        cursor.close()
